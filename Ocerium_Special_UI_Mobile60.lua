...
function Library.Main(text : string)
    ...
    game:GetService("UserInputService").InputBegan:Connect(function(key,istyping)
        if key.KeyCode.Name == Enum.KeyCode[Library["theme"]["HideKey"]].Name and not istyping then
            if opened then
                ...
                OnEffect:Play() 
                Main:TweenSize(UDim2.new(0.6, 0, 0.6, 0),"Out","Quart",0.6,true)
                for c,x in next,allproperties do
                    for b,n in next,allproperties[c] do
                        Main.Visible = true
                        if tostring(b) == "TextTransparency" then
                        ...
                        spawn(function()
                            ...
                            for i = 1,2 do
                                spawn(function()
                                    ...
                                    Line.Parent = Button
                                    Line.Position = (i == 1 and UDim2.new(1,-5,0,0) or i == 2 and UDim2.new(0, 3, 0, 0))
                                ...
                                wait(0.3)
                                    ...
                                    Line:TweenSizeAndPosition(UDim2.new(0, 1, 0, 0),(i == 1 and UDim2.new(1,-5,1,0) or UDim2.new(0,5,1,0)),"Out","Quint",1,true)
                                wait(0.5)
                                    Line:Destroy()
                                end)
                            end
    ...
    spawn(function()
        ...
        Main.Visible = true
        Main:TweenSize(UDim2.new(0.6, 0, 0.6, 0),"Out","Quart",1,true)
        for c,x in next,allproperties do
            for b,n in next,allproperties[c] do
                Tween(c,1,{[b] = n})
            end
